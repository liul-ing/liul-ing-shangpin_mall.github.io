            ...
            <div class="chooseArea">
              ...
              <dl v-for="attrs in spuSaleAttrList" :key="attrs.id">
                ...
                <dd
                  ...
                  v-for="item in attrs.spuSaleAttrValueList"
                  :key="item.id"
                  @click="changeActivate(item, attrs.spuSaleAttrValueList)"
                >
...
export default {
  ...
  computed: {
    ...mapGetters({ categoryView: "detail/categoryView" }),
    ...mapGetters({ skuInfo: "detail/skuInfo" }),
    ...mapGetters({ spuSaleAttrList: "detail/spuSaleAttrList" }),
    //给子组件的数据
    skuImageList() {
  ...
  methods: {
    //点击售卖属性时，点击谁谁就亮其余不亮---》排他思想
    //传的值是自己和自己的兄弟元素
    changeActivate(item, spuSaleAttrValueList) {
      //先干掉所有
      spuSaleAttrValueList.forEach((items) => {
        items.isChecked = 0;
      });
      //再给自己设置样式
      item.isChecked = 1;
    ...
    //加入购物车部分用户输入商品个数  要考虑合法性：非小数、非负数、不能包含字符
    changeSkuNum() {
      let value = this.skuNum * 1; //如果skuNum是字符格式，那么value值为NaN
      if (isNaN(value) && value < 1) {
        //带非法字符与数量小于1的情况
        this.skuNum == 1;
      } else {
        //数量大于1的情况（考虑小数）
        this.skuInfo = parseInt(value);
      }
    },
    //点击加入购物车按钮事件
    /*通知服务器保存数据，服务器要返回保存成功与否的结果，成功则跳转，错误给提示
      (1)dispacth语句相当于调用addOrUpdateShopCart函数，该函数返回的是Promise对象（async返回的一定是一个Promise）
      (2)服务器要返回保存成功与否的结果，成功则进行路由跳转，错误给用户提示
    */
    async addShopCar() {
      //跳转路由之前发请求，将商品id与数量通知服务器进行保存
      try {
        //表示成功
        ...
        //进行路由跳转，路由跳转时要将商品信息传给下一个路由组件(商品信息很多，是对象)
        // 方法一：
        // this.$router.push({
        //   path: "/addcartsuccess",
        //   query: { skuNum: this.skuNum, skuInfo: this.skuInfo },
        // });
        //上面这种方法进行路由传参是可以的，但是路径会不好看---》本地存储
        // 一些简单的数据如skuNum，通过query参数传递，较复杂的数据通过会话存储(不持久化，会话结束则数据消失)
        // 方法二：
