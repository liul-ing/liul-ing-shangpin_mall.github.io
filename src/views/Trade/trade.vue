  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        <p @click="changeStyle(address, addressInfo)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <!-- 商品信息 -->
        <ul class="list clearFix" v-for="order in orderInfo" :key="order.id">
          <li>
...
export default {
  name: "Trade",
  mounted() {
    this.$store.dispatch("trade/getAddressInfo"); //获取交易页面地址信息
    this.$store.dispatch("trade/getOrderInfo"); //获取交易页面商品信息
  },
  data() {
    return {
      msg: "", //留言板数据
      orderId: "", //用来接收提交订单请求成功返回的订单号
    };
  },
  computed: {
    ...mapState({
    ...
    //默认收件人的信息计算出来
    defaultUser() {
      //find:数组的方法,找到符合条件的元素.回调需要返回布尔值【真|假】，真即为查找结果【如果多个结果都为真，取其中一个】
      return this.addressInfo.find((item) => item.isDefault == "1") || {};
    },
  },

  methods: {
    changeStyle(address, addressInfo) {
      //点击谁，谁就有边框样式与默认地址--》排他思想，要传入点击的目标address及其兄弟addressInfo
